...
namespace phukienipadx.Core.Utilities
{
    public static class StringUtils
    {
        ...
        /// <returns></returns>
        public static string ParseSimpleUrl(this string url, string pageName)
        {
        ...
        public static string RemoveBadCode(this string input)
        {
            return input.RemoveBetween("<!--[if gte mso", "<!--[endif] -->");
        }

        private static string RemoveBetween(this string input, char begin, char end)
        {
            var regex = new Regex(string.Format("\\{0}.*?\\{1}", begin, end));
            return regex.Replace(input, string.Empty);
        }

        private static string RemoveBetween(this string input, string start, string end)
        {
        ...
        private static string RemoveSpecialCharacters(this string input)
        {
        ...
        private static string RemoveMarks(this string input)
        {
            var regex = new Regex(@"\p{IsCombiningDiacriticalMarks}+");
            string strFormD = input.Normalize(NormalizationForm.FormD);
        ...
        private static string SpaceToStroke(this string input)
        {
            input = input.Replace(' ', '-');
